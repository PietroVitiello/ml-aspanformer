...
from .correspondance_to_pose.correspondence_to_pose_svd import get_predicted_T
from .se3_tools import rot2rotvec
...
class PL_ASpanFormer(pl.LightningModule):
    ...
    def real_world_validation_step(self, batch):
        
        with self.profiler.profile("LoFTR"):
            self.matcher(batch)

        T_pred = get_predicted_T(batch)
        T_gt = batch["T_apriltag_gt"]

        R_errors = 0
        t_errors = 0
        bs = T_gt.shape[0]
        for i in range(bs):
            R_error = T_gt[i,:3,:3].cpu().numpy() @ np.linalg.inv(T_pred[i,:3,:3])
            R_errors += np.rad2deg(np.linalg.norm(rot2rotvec(R_error)))
            t_errors += np.linalg.norm(T_gt[i,:3,:3].cpu().numpy() - T_pred[i,:3,3])
        return R_errors/bs, t_errors/bs
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx):
        if dataloader_idx == 0:
            ...
            R_error, t_error = self.real_world_validation_step(batch)
            return {
                'R_error': R_error,
                't_error': t_error
            }
    ...
    def validation_epoch_end(self, outputs):
        ...
        ###################
        outputs = multi_outputs[1]
        # val_data["rw_R_error"] = outputs["R_error"]
        val_data["rw_R_error"] = [o['R_error'] for o in outputs]
        val_data["rw_t_error"] = [o['t_error'] for o in outputs]
        ...
        self.log("rw_R_error", val_data["rw_R_error"])
